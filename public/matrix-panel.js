...
export function createMatrixPanel({ api, getSession, onSession, onChange, requestId = makeRequestId, pollDelay = 1500, pollLimit = 120000 }) {
  ...
  async function perform(label, task, { allowRetry = false } = {}) {
    ...
    try {
      ...
      if (accepted) { error = ''; retry = null; schedule(); }
    }
    catch (failure) {
      if (sessionId !== id) return;
      error = failure.message || 'The Matrix connection could not be checked.';
      if (failure.uncertain) error += ' The request may already exist. Check its result before requesting another block.';
  ...
  function schedule() {
    clearTimeout(timer);
    if (!open || !result?.bridge?.connected || !result.bridge.demonstrations.some(item => ACTIVE.has(item.status))) return;
    ...
    timer = setTimeout(async () => {
      ...
      const item = result?.bridge.demonstrations.find(item => ACTIVE.has(item.status));
      if (!item) return;
      try {
        const value = await api(`${path(id)}/demonstrations/${encodeURIComponent(item.id)}`);
        if (currentGeneration !== generation) return;
        adopt(value, id); changed(); schedule();
      } catch (failure) {
        if (currentGeneration !== generation) return;
  ...
  async function action(name, data = {}) {
    ...
    if (name === 'request') {
      ...
      if (!bridge?.connected || !bridge.readiness?.canLaunch || !request || !bridge.binding || session.status !== 'active' || bridge.demonstrations.some(item => ACTIVE.has(item.status))) return;
      const id = session.id;
  ...
  function render(session, { lessonBusy = false } = {}) {
    ...
    const pending = demos.find(item => ACTIVE.has(item.status));

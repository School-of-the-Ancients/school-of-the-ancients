...
function validStore(value: unknown): asserts value is SchoolStore {
  ...
  for (const [id, raw] of Object.entries(value.sessions)) {
    requireValue(record(raw) && savedId(id) && raw.id === id && Number.isSafeInteger(raw.revision) && Number(raw.revision) >= 1 && ['explain', 'example', 'guided_practice', 'socratic_check', 'recap', 'ended'].includes(String(raw.stage)) && Array.isArray(raw.messages) && raw.messages.length <= 1000 && Array.isArray(raw.events) && raw.events.length <= 1000 && record(raw.lesson) && record(raw.mentor) && record(raw.artifact) && savedDate(raw.createdAt) && savedDate(raw.updatedAt) && savedDate(raw.savedAt), 500, 'invalid_store', 'Saved School session is invalid; existing data was preserved.');
    ...
    for (const message of raw.messages) requireValue(record(message) && savedId(message.id) && savedText(message.text) && savedDate(message.createdAt) && ['learner', 'mentor', 'system'].includes(String(message.role)), 500, 'invalid_store', 'Saved transcript is invalid; existing data was preserved.');
    for (const event of raw.events) requireValue(record(event) && savedId(event.id) && savedDate(event.createdAt) && savedText(event.details) && ['session_started', 'stage_changed', 'experiment_applied', 'turn_cancelled', 'turn_failed', 'turn_interrupted'].includes(String(event.type)), 500, 'invalid_store', 'Saved lesson event is invalid; existing data was preserved.');
  ...
  for (const [id, raw] of Object.entries(value.turns)) {
    requireValue(record(raw) && savedId(id) && raw.id === id && typeof raw.sessionId === 'string' && Object.hasOwn(value.sessions, raw.sessionId) && savedId(raw.requestId) && savedText(raw.input, 4000) && savedDate(raw.createdAt) && ['question', 'answer', 'advance'].includes(String(raw.kind)) && ['running', 'completed', 'failed', 'cancelled', 'interrupted'].includes(String(raw.status)), 500, 'invalid_store', 'Saved turn is invalid; existing data was preserved.');
    ...
    if (raw.status === 'completed') requireValue(savedText(raw.output, 8000) && record(raw.receipt) && raw.receipt.completedTurn === true && raw.receipt.toolCallCount === 0 && ['demo', 'codex-cli'].includes(String(raw.receipt.mode)), 500, 'invalid_store', 'Saved provider receipt is invalid; existing data was preserved.');
  }
  for (const [id, raw] of Object.entries(value.receipts)) {
    requireValue(savedId(id) && record(raw) && typeof raw.fingerprint === 'string' && /^[a-f0-9]{64}$/.test(raw.fingerprint) && typeof raw.resourceId === 'string' && ['start', 'turn', 'cancel', 'experiment'].includes(String(raw.kind)), 500, 'invalid_store', 'Saved receipt is invalid; existing data was preserved.');
    requireValue(Object.hasOwn(['turn', 'cancel'].includes(String(raw.kind)) ? value.turns : value.sessions, raw.resourceId), 500, 'invalid_store', 'Saved receipt points to a missing record; existing data was preserved.');
  }
}

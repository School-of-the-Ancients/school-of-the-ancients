...
export function parseCodexResult(raw: string, final: string): MentorResult {
  ...
  try {
    ...
    requireValue(completed && typeof lastMessage === 'string' && JSON.stringify(JSON.parse(lastMessage)) === JSON.stringify(result) && result && Object.keys(result).length === 1 && typeof result.text === 'string' && result.text.trim().length > 0 && result.text.length <= 8000 && !/[\u0000-\u0008\u000b\u000c\u000e-\u001f]/.test(result.text), 502, 'provider_invalid', 'Codex returned incomplete or invalid teaching text.');
    return { text: result.text.trim(), receipt: { mode: 'codex-cli', completedTurn: true, toolCallCount: 0, ...(actualModel ? { actualModel } : {}) } };
...
export class CodexMentorProvider implements MentorProvider {
  ...
  async respond(input: MentorInput, signal: AbortSignal): Promise<MentorResult> {
    ...
    try {
      ...
      writeFileSync(schemaPath, JSON.stringify({ type: 'object', additionalProperties: false, required: ['text'], properties: { text: { type: 'string', minLength: 1, maxLength: 8000 } } }));
